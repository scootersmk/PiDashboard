...
@click.command()
@click.option('--mode', type=click.Choice(["once", "forever"]), default="once", help="Run mode: once gathers data once, and then exits, forever gathers data every INTERVAL seconds forever")
@click.option('--interval', default=600, help="Number of seconds between data queries, default is 600 seconds(5 minutes)")
@click.option('--api_key', required=True, help="API Key from openweathermap.org", envvar="API_KEY")
@click.option('--units', type=click.Choice(["imperial", "metric"]), default="imperial", help="Type of units to get from API")
@click.option('--location-id', default="5391959", help="Location Id, default to 5391959 for San Francisco, CA")
@click.option('--logfile', default="weather.log", help='Log filename data is written to.')
